...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const populateMatchData = (eventId) => {
        ...
        if (goalscorerSelections) {
            goalscorerSelections.forEach(selection => {
                ...
                if (event.source === 'Cloudbet' && selection.status === 'SELECTION_ENABLED') {
                    playerName = parsePlayerNameFromOutcome(selection.outcome);
                    const playerInfo = Object.values(event.players || {}).find(p => p.name === playerName);
                    if(playerInfo) teamSide = playerInfo.team.toLowerCase();
                } else if (event.source === 'TheOddsAPI') {
                     playerName = selection.description; 
                }

                if(playerName){
                    availableApiPlayers.push({ name: playerName, goalscorerOdd: selection.price, teamSide });
                }
            });
        }
        ...
        let formattedApiText = JSON.stringify(event, null, 2);
        apiDataDisplay.querySelector('pre').textContent = formattedApiText;
    };
    ...
    on(teamSelect, 'change', e => {
        ...
        if (teamName && teamName !== 'all' && e.target.value !== 'all' && window.injuryManager) {
        ...
        if (teamName && teamName !== 'all' && e.target.value !== 'all' && window.lineupManager) {
        ...
        const playersToDisplay = selectedTeamSide === 'all' 
            ? availableApiPlayers 
            : availableApiPlayers.filter(p => p.teamSide === selectedTeamSide || p.teamSide === 'unknown'); // Show 'unknown' for both
