export default class InjuryManager {
    ...
    createDetailedDisplay(teamName) {
        ...
        return `<div class="injury-panel has-injuries"><div class="injury-header"><h4>🏥 Injury Report (${injuries.length})</h4></div><div class="injury-list">${injuryList}</div></div>`;
    }
    ...
    addStyles() {
        ...
        styles.textContent = `.injury-panel { border-radius: 10px; padding: 20px; margin: 15px 0; border: 1px solid #e9ecef; animation: slideIn 0.4s ease; box-shadow: 0 2px 8px rgba(0,0,0,0.1); } @keyframes slideIn { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } } .injury-panel.safe { background: linear-gradient(135deg, #d4edda, #c3e6cb); border-color: #28a745; } .injury-panel.has-injuries { background: linear-gradient(135deg, #f8d7da, #f5c6cb); border-color: #dc3545; } .injury-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; } .injury-header h4 { margin: 0; color: #2c3e50; font-size: 18px; } .injury-list { max-height: 250px; overflow-y: auto; margin: 10px 0; } .injury-card { background: rgba(255,255,255,0.9); border-radius: 8px; padding: 15px; margin: 8px 0; border-left: 4px solid; backdrop-filter: blur(5px); transition: transform 0.2s; } .injury-card:hover { transform: translateX(3px); } .injury-card.severe { border-left-color: #dc3545; } .injury-card.moderate { border-left-color: #fd7e14; } .injury-card.minor { border-left-color: #ffc107; } .injury-card.doubtful { border-left-color: #6f42c1; } .player-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; } .player-name { font-weight: 600; color: #2c3e50; font-size: 15px; } .player-position { background: #e9ecef; padding: 3px 8px; border-radius: 10px; font-size: 11px; color: #6c757d; font-weight: 500; } .injury-info { display: flex; justify-content: space-between; font-size: 13px; gap: 10px; } .injury-type { color: #dc3545; font-weight: 500; flex: 1; } .return-info { color: #6c757d; white-space: nowrap; } .no-injuries { text-align: center; color: #28a745; font-weight: 600; margin: 0; font-size: 16px; }`;
